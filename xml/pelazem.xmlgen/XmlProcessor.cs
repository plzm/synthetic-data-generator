...
namespace pelazem.xmlgen
{
	internal class XmlProcessor
	{
		...
		internal XmlProcessor(string url, XmlQualifiedName rootElem) : this(XmlSchema.Read(new XmlTextReader(url), new ValidationEventHandler(ValidationCallBack)), rootElem)
		{
		}

		internal XmlProcessor(XmlSchema schema, XmlQualifiedName rootElem)
		{
			if (schema == null)
			{
		...
		internal XmlProcessor(XmlSchemaSet schemaSet, XmlQualifiedName rootElem)
		{
		...
		private void GenerateAttributeWildCard(XmlSchemaComplexType ct, InstanceElement elem)
		{
			...
			if (attributeWildCard.ProcessContents == XmlSchemaContentProcessing.Skip || attributeWildCard.ProcessContents == XmlSchemaContentProcessing.Lax)
			{
				...
				if (attr != null)
				{
					attr.ValueGenerator = XmlValueGenerator._anySimpleTypeGenerator;
